        ...
        <div className="container px-5 py-24 mx-auto flex flex-wrap flex-col">
          ...
          <div className="flex mx-auto mb-10 justify-center">
            <ScrollArea className="w-96 lg:w-2/3 whitespace-nowrap rounded-md">
              {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
                <a 
                  ...
                  ${activeProjectGroup === each.title ? 
                    "bg-[#2D2926] dark:bg-[#ed6f63] text-[#ed6f63] dark:text-[#2D2926] border-[#ed6f63] dark:border-[#2D2926] rounded-t" : 
                    "border-[#2D2926] dark:border-[#ed6f63]"}
                  `}
              ...
              <ScrollBar orientation="horizontal" className="bg-[#2D2926] dark:bg-[#ed6f63]" />
            </ScrollArea>
          </div>
          
          {Redux.state.ReceivedObject?.Retrieve?.ProjectSectionRetrieve?.projectGroups?.map((each, index) => (
            <React.Fragment key={index}>
              <div className={`flex flex-wrap text-center bg-[#2D2926] dark:bg-[#ed6f63] text-[#ed6f63] dark:text-[#2D2926] bg-opacity-60 px-4 py-6 rounded-lg ${activeProjectGroup === each.title ? "" : "hidden"}`}>
                ...
                {each?.projects?.map((each1, index1) => (
                  <React.Fragment key={index1}>
                    <div className="p-4 w-full md:w-1/3 text-left">
                      <div className="h-full bg-[#ed6f63] dark:bg-[#2D2926] text-[#2D2926] dark:text-[#ed6f63] rounded-lg overflow-hidden">
                        ...
                        <div className="p-6">
                          ...
                          <div className="flex items-center flex-wrap ">
                            ...
                            <span className="mr-3 inline-flex items-center lg:ml-auto md:ml-0 ml-auto leading-none text-sm pr-3 py-1 border-r-2 border-[#2D2926] dark:border-[#ed6f63]">
